...
['strength', 'dexterity', 'constitution', 'intelligence', 'wisdom', 'charisma'].forEach(attr => {
    on(`change:${attr}`, function() {
        ...
        (attr === "strength" || attr === "constitution") ? update_weight(): false;
    ...
    on(`change:${attr}_mod`, function() {
        ...
        switch (`${attr}`) {
            ...
            case "dexterity":
                ...
                break;
            case "intelligence":
                update_skills(["arcana", "history", "investigation", "nature", "religion", "wile"]);
                update_initiative();
                break;
            case "wisdom":
...
var update_weight = function() {
    ...
    var weight_attrs = ["cp", "sp", "ep", "gp", "pp", "currency_value", "encumberance_setting", "strength", "strength_mod", "constitution", "constitution_mod", "size", "carrying_capacity_mod", "inventory_slots_mod", "use_inventory_slots", "itemweightfixed", "itemslotsfixed"];
    getSectionIDs("repeating_inventory", function(idarray) {
        _.each(idarray, function(currentID, i) {
        ...
        getAttrs(weight_attrs, function(v) {
            ...
            if (v["use_inventory_slots"] == 1) {
                ...
                attr_slots = Math.max(parseInt(v.strength_mod, 10), parseInt(v.constitution_mod, 10));
                size_slots += attr_slots;
...
var update_npc_skills = function() {
    getAttrs(["npc_acrobatics_base", "npc_animal_handling_base", "npc_arcana_base", "npc_athletics_base", "npc_rhetoric_base", "npc_history_base", "npc_insight_base", "npc_intimidation_base", "npc_investigation_base", "npc_medicine_base", "npc_nature_base", "npc_perception_base", "npc_performance_base", "npc_wile_base", "npc_religion_base", "npc_sleight_of_hand_base", "npc_stealth_base", "npc_survival_base"], function(v) {
        ...
        if (v.npc_wile_base && v.npc_wile_base != "@{intelligence_mod}") {
            wile = parseInt(v.npc_wile_base, 10);
            if (last_skill === 0) {
                last_skill = 1;
                wile_flag = wile < 0 ? 4 : 2;
            } else {
                wile_flag = wile < 0 ? 3 : 1;
            }
        } else {
            wile_flag = 0;
            wile = "";
        ...
        update["npc_wile"] = wile;
        update["npc_wile_flag"] = wile_flag;
